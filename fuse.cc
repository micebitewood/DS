                   ...
                   struct fuse_file_info *fi)
{
    ...
    ret = getattr(inum, st);
    if(ret != yfs_client::OK){
        std::cout << "cannot get attribute for: " << inum << std::endl;
                ...
                off_t off, struct fuse_file_info *fi)
{
  ...
  std::string buf;
  std::string content;
  std::cout << "read size: " << size << ", off: " << off << std::endl;
  int ret = yfs->read(ino, content);
  if (ret != yfs_client::OK) {
    fuse_reply_err(req, ENOENT);
    return;
  }
...
#if 1
  // Change the above line to "#if 1", and your code goes here
  std::cout << "write: " << buf << ", to " << ino << ", size " << size << ", from " << off << std::endl;
  std::string bufString = std::string(buf, size);
  int ret = yfs->write(ino, bufString, size, off);
                        ...
                        mode_t mode, struct fuse_entry_param *e)
{
    ...
    std::string buf;
    yfs_client::inum ino;
    int ret = yfs->create(parent, name, ino, true);
    if (ret == yfs_client::OK) {
        e->ino = ino;
...
{
    std::cout << "fuseserver_create" << std::endl;
...
fuseserver_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
  ...
  std::cout << "lookup " << name << " in " << parent << std::endl;
  yfs_client::inum ino;
  int ret = yfs->lookUp(parent, name, ino);
  if (ret == yfs_client::OK) {
...
#if 1
    yfs_client::inum ino;
    int ret = yfs->create(parent, name, ino, false);
    if (ret == yfs_client::EXIST) {
        fuse_reply_err(req, EEXIST);
        return;
