...
{
  std::cout << " sync with backups start" << std::endl;
  ...
  std::cout << " sync with backups getting mutex" << std::endl;
  ...
  std::cout << " sync with backups, sleep" << std::endl;
  if (backups.size() > 1 || std::find(backups.begin(), backups.end(), cfg->myaddr()) == backups.end())
    pthread_cond_wait(&recovery_cond, &rsm_mutex);
  std::cout << " sync with backups, waken up" << std::endl;
  insync = false;
  return true;
}
...
rsm::sync_with_primary()
{
  ...
  bool ret;
  insync = true;
  while ((ret = statetransfer(m)) == false) {
    if (vid_insync != vid_commit) {
      std::cout << " sync with primary, there is a commited viewchange" << std::endl;
      return false;
    }
  }
  insync = false;
...
rsm::statetransferdone(std::string m) {
  ...
  rpcc *cl = h.safebind();
  int r;
  int ret = cl->call(rsm_protocol::transferdonereq, cfg->myaddr(), vid_insync, r, rpcc::to(1000));
  if (ret == rsm_protocol::BUSY) {
      std::cout << " statetransferdone, primary is busy" << std::endl;
    return false;
  }
  std::cout << " statetransferdone, return true" << std::endl;
  return true;
}
...
{
  ScopedLock ml(&invoke_mutex);
  std::cout << "in client invoke, vid_commit: " << vid_commit << ", myaddr: " << cfg->myaddr() << std::endl;
  int ret = rsm_client_protocol::OK;
  if (inviewchange) {
    std::cout << "inrsm: in view change" << std::endl;
    ret = rsm_client_protocol::BUSY;
  } else if (!amiprimary()) {
    std::cout << "inrsm: not primary" << std::endl;
    ret = rsm_client_protocol::NOTPRIMARY;
  } else {
    std::vector<std::string>::iterator it;
    std::vector<std::string> members = cfg->get_view(vid_commit);
    std::cout << "inrsm: members size " << members.size() << std::endl;
    for (it = members.begin(); it != members.end(); it++) {
      std::cout << "inrsm: member: " << *it << std::endl;
      if (*it == cfg->myaddr())
        continue;
    //for (it = backups.begin(); it != backups.end(); it++) {
      ...
      rpcc *cl = h.safebind();
      int dummy;
      rsm_protocol::status res = cl->call(rsm_protocol::invoke, procno, myvs, req, dummy, rpcc::to(1000));
      std::cout << *it << ": " << res << std::endl;
      if (res != rsm_protocol::OK) {
        std::cout << "inrsm: some replica doesn't respond: " << *it << std::endl;
        ret = rsm_client_protocol::BUSY;
        break;
      }
      breakpoint1();
    ...
    if (ret != rsm_client_protocol::BUSY) {
      ...
      last_myvs = myvs;
        std::cout << "inrsm: execute done" << std::endl;
    }
  }
  return ret;
}
...
rsm::invoke(int proc, viewstamp vs, std::string req, int &dummy)
{
  ...
  if (inviewchange) {
    ...
    std::cout << " viewstamp different" << std::endl;
...
rsm_protocol::transferres &r)
{
  ...
  if (!insync || vid != vid_insync) {
    std::cout << "transferreq return busy because " << insync << ", " << true << "; " << vid << ", " << vid_insync << std::endl;
...
rsm::transferdonereq(std::string m, unsigned vid, int &)
{
  ...
  if (!insync || vid != vid_insync) {
    std::cout << " transferdonereq, I'm busy" << std::endl;
    return rsm_protocol::BUSY;
  }
  std::vector<std::string>::iterator it;
  std::cout << " transferdonereq, src " << m  << ", backups size " << backups.size() << std::endl;
  if ((it = std::find(backups.begin(), backups.end(), m)) != backups.end()) {
    std::cout << " transferdonereq, Gotcha" << std::endl;
    backups.erase(it);
    if (backups.size() == 1) {
      std::cout << " transferdonereq, signal!" << std::endl;
      pthread_cond_signal(&recovery_cond);
    }
  }
  std::cout << " transferdonereq, OK" << std::endl;
  return ret;
}
